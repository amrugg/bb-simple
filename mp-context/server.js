...
var port = (params.port || params.p) || (params.ssl ? 443 : 2700);
...
var setsDir = p.join(__dirname, 'sets');
if (!fs.existsSync(setsDir)) fs.mkdirSync(setsDir);
function getSetFile(setName) {
    return p.join(setsDir, setName + '.json');
}
createServer(function(req, res)
{
    ...
    if (pathname === '/sets') {
        fs.readdir(setsDir, function(err, files) {
            ...
            var sets = files.filter(function(f) { return /\.json$/.test(f); })
    ...
    if (pathname === '/load') {
        ...
        var set = parsedUrl.query.split("=")[1];
        if (!set) {
            res.writeHead(400, {'Content-Type': 'text/plain'});
            res.end('No set specified');
            return;
        }
        var file = getSetFile(set);
    ...
    if (pathname === '/save' && req.method === 'POST') {
        ...
        req.on('end', function() {
            try {
                ...
                fs.writeFile(getSetFile(set), JSON.stringify(json, null, 4), function(err) {
                    if (err) {
